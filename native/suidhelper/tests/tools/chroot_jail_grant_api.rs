//! Contracts of the `chroot-jail grant-api` op. The lexical refusal contracts
//! (absoluteness, strict components, exact leaf name) are enforced at clap-parse
//! time by `ApiSocket`'s `FromStr` and are tested there; the base-relative shape
//! and the privileged filesystem behavior are driven through the base-injected
...
//!     leaf that is not exactly `api.socket` is rejected constructing an
//!     `ApiSocket`, before any filesystem access;
//!   * shape — a valid `ApiSocket` is granted iff it is exactly
//!     `<exec>/<id>/root/api.socket` below the jail base; any other depth is
...
use hyper_suidhelper::tools::chroot_jail::grant_api::{
    grant_api_under, ApiSocket, Error, GrantOut,
};
...
/// Parse an absolute socket path into a validated `ApiSocket`, panicking if the
/// lexical gate rejects it — for tests whose subject is the base-relative or
/// filesystem behavior, not the parse.
fn sock(p: &Path) -> ApiSocket {
    ApiSocket::from_str(p.to_str().unwrap()).unwrap()
}
...
#[test]
fn wrong_leaf_basename_is_rejected_at_parse() {
    let tmp = tempfile::tempdir().unwrap();
    let bad = tmp.path().join("exec/id/root/evil.sock");
    let err = ApiSocket::from_str(bad.to_str().unwrap()).unwrap_err();
...
fn dotdot_traversal_is_rejected_at_parse() {
    let tmp = tempfile::tempdir().unwrap();
    let bad = format!("{}/exec/../id/root/api.socket", tmp.path().display());
    let err = ApiSocket::from_str(&bad).unwrap_err();
    assert!(
...
fn relative_socket_is_rejected_at_parse() {
    let err = ApiSocket::from_str("exec/id/root/api.socket").unwrap_err();
    assert!(
...
fn socket_outside_jail_base_is_rejected() {
    ...
    let err = grant_api_under(&jail, &sock(&outside)).unwrap_err();
    assert!(
...
fn too_shallow_is_shape_error() {
    ...
    let err = grant_api_under(jail, &sock(&bad)).unwrap_err();
...
fn missing_socket_is_pending() {
    ...
    let out = grant_api_under(jail, &sock(&socket)).expect("missing socket must be Ok(Pending)");
...
fn missing_jail_tree_is_pending() {
    ...
    let out = grant_api_under(jail, &sock(&socket)).expect("half-built jail must be Ok(Pending)");
    assert!(matches!(out, GrantOut::Pending), "got {out:?}");
}
...
fn real_socket_is_granted_and_chmod_0660() {
    ...
    let out = grant_api_under(jail, &sock(&socket)).expect("real socket must grant");
...
fn regular_file_at_leaf_is_refused_and_untouched() {
    ...
    let err = grant_api_under(jail, &sock(&imposter)).unwrap_err();
...
fn symlink_at_leaf_is_refused() {
    ...
    let err = grant_api_under(jail, &sock(&link)).unwrap_err();
    assert!(matches!(err, Error::NotASocket), "got {err:?}");
}
...
fn symlinked_component_does_not_escape() {
    ...
    let _ = grant_api_under(&jail, &sock(&socket)); // O_NOFOLLOW makes the walk refuse
...
proptest! {
    ...
    // the lexical parse always succeeds and the leaf is always `api.socket`.
    #[test]
    fn shape_classification(
        ...
        let res = grant_api_under(jail, &sock(&socket));
        if parents.len() == 3 {
