...
defmodule Mix.Tasks.Compile.SuidhelperStamp do
  ...
  Steps, run before the Elixir compiler so the generated module compiles:
    ...
    2. The stamped binary's `version` subcommand is invoked; its JSON
       ...
       build identity.
    3. That JSON is baked into `lib/hyper/suid_helper/expected.ex` so the BEAM
       can compare a deployed helper against the one this build produced.
  ...
  defp generate(json) do
    ...
    defmodule Hyper.SuidHelper.Expected do
      ...
      @doc "Raw `version` JSON the helper reported when this build was stamped."
      @spec json() :: String.t()
      def json, do: @json
