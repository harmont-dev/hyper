defmodule Hyper.Node.Img.Server do
  ...
  defp build_chain(img_id, [base | deltas]) do
    with {:ok, sectors} <- SuidHelper.device_sectors(base) do
      deltas
      |> Enum.with_index(1)
      |> Enum.reduce_while({:ok, base, []}, fn {cow_dev, idx}, {:ok, origin, names} ->
        name = dm_name(img_id, idx)

        case SuidHelper.dmsetup_create_snapshot(name, origin, cow_dev, sectors) do
          {:ok, dev} ->
  ...
  defp remove_quietly(name) do
    case SuidHelper.dmsetup_remove(name) do
      :ok ->
        :ok
