defmodule Hyper.Node.Img.Publish do
  ...
  composed RO image device, backed by a fresh sparse COW file → diff-copy the
  thin snapshot against the RO device into it, so exactly the divergent chunks
  land in the COW exception store → tear the devices down (flushing the store)
  → ingest the COW file via `Hyper.Img.create_derived/3`.
  ...
  defp materialize(%{thin_name: origin_name, thin_id: origin_id, origin_dev: ro_dev}, img_id) do
    ...
    with {:ok, sectors} <- SuidHelper.Blockdev.device_sectors(ro_dev),
      ...
      try do
        write_delta(tmp, ro_dev, snap_dev, sectors, img_id)
      after
  ...
  @spec write_delta(String.t(), Path.t(), Path.t(), pos_integer(), Hyper.Img.id()) ::
          {:ok, Hyper.Img.id()} | {:error, term()}
  defp write_delta(tmp, ro_dev, snap_dev, sectors, parent_img_id) do
